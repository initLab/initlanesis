...
if ( !function_exists( 'get_scpt_formatted_meta' ) ) {
	...
	function get_scpt_formatted_meta( $key, $post_id = false ) {
		global $scpt_known_meta, $scpt_known_custom_fields;		if ( false == $post_id )
			$post_id = get_the_ID();		if ( isset( $scpt_known_meta[ $post_id ][ $key ] ) )
			return $scpt_known_meta[ $post_id ][ $key ];		$value = get_post_meta( $post_id, $key );
		if ( !$value || !is_array( $value ) ) return set_known_scpt_meta( $key, $value, $post_id );		if ( ! $field_info = get_known_field_info( $key, $post_id ) )
			return set_known_scpt_meta( $key, $value[0], $post_id );		if ( is_array( $field_info ) ) {
			if ( $field_info['data'] ) {

...
/*------------------------------------------------------------*/
	...
	// remove indicator on items with out sub-menu
	jQuery( ".mobile_menu ul.menu li" ).each(function( i ) {
		var has_sub = jQuery(this).children('.sub-menu').length;
		
		if( has_sub == 0){
			jQuery(this).children('.zp_menu_indicator').remove();
		}
	});	

	// menu trigger

	jQuery(".menu_trigger").toggle(function(){
		jQuery('.mobile_menu #nav').slideDown();
	}, function () {
		jQuery('.mobile_menu #nav').slideUp();
	});
	
	
	// menu item trigger to display sub menu
	jQuery('.mobile_menu .sub-menu').hide();
	jQuery('.mobile_menu .zp_menu_indicator').toggle(function() {		

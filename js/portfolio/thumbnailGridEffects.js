(function() {
	...
	function init() {
		...
		[].forEach.call( document.querySelectorAll( '.zp-grid-wrapper' ), function( el ) {

			var grid = el.querySelector( '.zp-grid' ),
				items = [].slice.call( grid.querySelectorAll( 'div' ) ),
				navDots = [].slice.call( el.querySelectorAll( '.zp_portfolio_nav > a' ) ),
				isAnimating = false,
				current = 0;
			...
			function updateCurrent( set ) {
				classie.remove( navDots[ current ], 'zp-current' );
				classie.add( navDots[ set ], 'zp-current' );
				current = set;
			}
			...
			function loadNewSet( set ) {
				...
				items.forEach( function( el ) {
					...
					// add class "zp-old" to the elements/images that are going to get removed
					if( itemChild ) {
						classie.add( itemChild, 'zp-old' );
					}
				} );
				...
				setTimeout( function() {
					...
					classie.add( grid, 'zp-effect-active' );
					...
					var onEndAnimFn = function() {
						...
						items.forEach( function( el ) {
							// remove old elems
							var old = el.querySelector( 'a.zp-old' );
							if( old ) { el.removeChild( old ); }
							// remove class "zp-empty" from the empty items
							classie.remove( el, 'zp-empty' );
							// now apply that same class to the items that got no children (special case)
							if ( !el.hasChildNodes() ) {
								classie.add( el, 'zp-empty' );
							};
							 pretty();
							
						} );
						// remove the "effect" class
						classie.remove( grid, 'zp-effect-active' );
						isAnimating = false;
					};
					if( support ) {
